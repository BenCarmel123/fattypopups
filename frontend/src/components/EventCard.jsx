...
function ActionButton({ children, onClick, ariaLabel, className = '' }) {
  return (
    ...
    style={{ color: GRAY, minHeight: '2.6rem', minWidth: '2.2rem', padding: '0.45rem', background: TRANSPARENT, transition: 'background 0.18s' }}
    ...
    <Drawer.Root> 
        ...
        <Drawer.Trigger asChild>
         <ActionButton ariaLabel={DETAILS}>
           <LuSquarePlus style={{ verticalAlign: 'middle', marginRight: '-0.2rem' }} />
...
export default function EventCard({ event }) {
  ...
  .eventcard-actions{ display:flex; align-items:center; justify-content:space-between; gap:0.64rem; width:100%; max-width:420px; margin:0 auto; box-sizing: border-box; }
      ...
      <div style={{ width: MAX, display: BLOCK }}>
        ...
        <Card.Description fontSize={XL} color="gray.600" pt={2} paddingRight={4} paddingLeft={4} lineHeight={2.5} >
          ...
          <span className={ACTION_BUTTON_HOVER} onClick={() => handleCalendar(event)}>
  ...
  <div className="eventcard-actions" style={{display: FLEX, alignItems: CENTER, justifyContent: CENTER, gap: '0.01rem', width: MAX, maxWidth: '340px', margin: '0 auto'}}>
          ...
          <ActionButton onClick={() => handleWhatsApp(event.english_description)} ariaLabel="Share">
            <TbSend style={{ verticalAlign: 'middle', marginRight: '-0.3rem' }} />
            <p className="eventcard-action-text" style={{display: 'inline-flex', alignItems: 'center', whiteSpace: 'nowrap'}}>{SHARE}</p>
          </ActionButton>
